...
Since the Planck final release will likely be the best full-sky survey from space available to us in the next decade, it is important to extract all the information present in the data. In light of the $\texttt{SRoll2}$ likelihood release, we obtain new reionization PC constraints with this latest likelihood. Enabled by the completeness property of the PCs, we also turn these constraints into an effective likelihood useful for assessing the CMB likelihood of \textit{any} reionization model out to $\zmax$ = 30, following techniques tested in Ref.~\cite{Heinrich:2016ojb}. 

The code is publicly available on GitHub at \url{https://github.com/chenheinrich/RELIKE}. We publish a python package \texttt{relike}, which can be used to evaluate the Planck likelihood of any reionization model. If desired, this package can be connected to a MCMC sampler to sample the posterior of higher-dimensional parameter space; we provide the CosmoMC sampler with a fortran implementation of the `relike` likelihood in \texttt{CosmoMC-relike}. We expect the code release to dramatically reduce the time for testing individual models, and to provide consistent yet fast way of modeling the ionization history in joint analyses of Planck with other reionization datasets such as luminosity function, star formation history, 21cm or line-intensity mapping in general, etc.
...
\caption{Changes in the best constrained $m_1-m_2$ PC plane between Planck 2015 (red), Planck 2018 (blue, \texttt{SRoll2}, and Planck 2018 but with the \texttt{SimAll} likelihood (green). With Planck 2018, the \texttt{SRoll2} likelihood moderately improves the best constrained direction and raises the optical depth.  \todo{add SRollv2. convention: put labels only when we do comparisons}}
...
Instead of the KDE likelihood, we can alternatively use the Gaussian approximation given in Eq.~(\ref{eq:gaussian}). The Gaussian likelihood is faster to evaluate as it does not require looping over the entire $\sim 10^6$ points in the PC chain. {[add timing here]} 
...
%
% longer KDE and Gaussian chains
...
suggested change in model - or find another reason for plotting the currently chosen one. \ch{updated plot with longer exact chains (still running)}
}
...
\todo{change z1 z2 to zre and zhi everywhere.}
...
&+& \frac{\xemin - x_e^{\rm rec}}{2}\left\{  1+ \tanh\left[ \frac{z_{\rm t}-z}{\Delta z_{2}} \right] \right\} + x_e^{\rm rec},
 ...
 \eea
where $y(z)=(1+z)^{3/2}$, $\Delta y=(3/2)(1+z_1)^{1/2}\Delta z_1$ where $\Delta z_1 = 0.015\, (1+z_{1})$ just as in the first example for a sharper step.
We choose the second step at $z_{2}=28$ with $\Delta z_2 = 1.0$, since the effective likelihood is limited to models with ionization below $\zmax=30$. 
In comparison to tanh, a single parameter $\xemin$ is added in this model to control the high-$z$ ionization plateau for $z_{1} \lesssim z \lesssim z_2$. We would recover the standard tanh as $\xemin$ approaches the negligible recombination value $x_e^{\rm rec}$. We show examples
...
{add timing results somewhere}

The marginalized 1D constraints from the KDE and Gaussian likelihoods 
\beq
\taulo = 0.0525^{+0.0062}_{-0.0079}, \; \tauhi < 0.021 \quad (95\% \text{ C.L., KDE)}
\eeq 
and
\beq
\taulo = 0.0528^{+0.0064}_{- 0.0077}, \; \tauhi < 0.021 \quad (95\% \text{ C.L., Gaussian)}
\eeq 
are consistent with those from the exact likelihood 
\beq
\taulo = 0.0526^{+0.0058}_{-0.0078}, \; \tauhi < 0.021 \quad (95\% \text{ C.L., Exact).
\eeq 
}
%exact: tauhi < 0.0206
%KDE: tauhi < 0.0214
...
\ch{NEW: $\tau_{\rm hi}$ upper limits are all one-sided already.}\\
...
and 
\ch{$\tau_{2\text{step}}(15, 30) < 0.016$} for both effective likelihoods. %changed from <0.017--> <0.016 when going from two-sided to one-sided.
...
% rephrased to flow with the next paragraph of 2015->2018
...
%
\beq
%\tau_{\rm PC,\, \zmax=50} = 0.0626^{+0.0061}_{-0.0072}\;\; (\texttt{SRoll2}).
...
%\beq
%\tau_{\rm PC,\, \zmax=30} = 0.0619^{+0.0056}_{-0.0068}\;\; (\texttt{SRoll2}),
%\eeq
%and
%\beq
%\tau_{\rm PC,\, \zmax=50} = 0.0626^{+0.0061}_{-0.0072}\;\; (\texttt{SRoll2}).
%\eeq
%
%Compared the \texttt{SimAll} likelihood results, 
%\beq
%\tau_{\rm PC,\, \zmax=30} = 0.0582 ^{+0.0072}_{-0.0083}\;\; (\texttt{SimAll}),
%\eeq
%and
%\beq
%\tau_{\rm PC,\, \zmax=50} = 0.0582^{+0.0077}_{-0.0088}\;\; (\texttt{SimAll}).
%\eeq
%the \texttt{SRoll2} error bars on PC $\tau$ was reduced by about 20\%, and the central values rose by about half a sigma. [wanna comment more here?]
%
...
%\ch{Planck 2018} \wh{decide on convention - in many places the default analysis isn't labeled, only changes from the default}
...
[Any comments on Ahn et al. \url{https://arxiv.org/pdf/2011.03582.pdf} and \url{https://arxiv.org/pdf/2006.16828.pdf}?]
