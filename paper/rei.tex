...
The principal component technique for constraining reionization using the large-angle $C_\ell^{EE}$ polarization spectrum was first introduced in Ref.~\cite{Hu:2003gh}. We now briefly summarize the PC technique in \S~\ref{sec:PC}, as well as the kernel density estimate technique for building the effective likelihood in \S~\ref{sec:KDE}. We refer the readers to Refs.~\cite{
...
%As we shall see in \S \ref{sec:modelindependent} this is also captured in our model independent approach to cumulative optical depth constraints. 
}
...
% Not sure we need to get into this subtlety - the caveat being that getdist will smooth the boundary even if it is in reality sharper.   Not sure we know which effect dominates and it probably doesn't matter much:
%
...
% Move this somewhere into the prior discussion or the 
% paper discussion
%
...
%\caption{Direct MCMC chains of the two-parameter model with \tauhi and \taulo: triangle plot of \tauhi and \taulo. The marginalized 1D constraints are \taulo = $0.0516 \pm 0.0076$, \tauhi = $0.0100 \pm   0.0066$. The ML model is \taulo = 0.0543 and \tauhi = 4 corresponding to $z_{\rm re} = 7.68$ and $x_{e, \mathrm{min}} = 0.021$.
%}
...
\caption{Posterior for the total optical depth in the PC analysis (shaded) and for a tanh (black solid line) and two-step model (blue dashed line)}. 
...
% this sounds too technical now for an intro to the section:
%As discussed in \S \ref{sec:cumulative}, since the PC amplitudes are not intrinisically restricted to physically sensible ionization histories where $0 \le x_e(z) \le x_e^{\rm max}$, this requires imposing physicality priors using Eq.~\ref{eq:individualprior}.
...
%On the other hand the tanh model provides an explicitly physical example whose trajectory runs close  and nearly parallel to the our physicality prior in the allowed region and so we do not expect the bias incurred by 
...
\wh{drop the ``with prior" label since with prior is our default approach}
}
