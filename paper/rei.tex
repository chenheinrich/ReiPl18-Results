...
\title{\ch{RELIKE}: Reionization Effective Likelihood from Planck 2018 Data (?)} 
...
suggested change in model - or find another reason for plotting the currently chosen one. \ch{updated plot with longer exact chains coming up}
}
...
\ch{comment here about 2D 95\% model picked with tau hi = 0.026 is higher because 0.021 is the 1D marginalized result.}
...
\ch{$\tau_{2\text{step}}(15,30)= 0.017$.}\todo{Need to change all 95\% upper limit on tau(15,30) to a one-sided computation instead of 2.}\todo{update plot two-step with longer chain results, run 2 more days exact two-step chains.} \todo{point out in text that 95\% upper limit on tau(15,30) not the same as plot.} 
...
More generally, to the extent that the data constrain $m_1$ and $m_2$ better than the flat range-bound priors, these priors become irrelevant and the reweighting of Ref.~\cite{Millea:2018bko} becomes erroneously motivated.   To see this, consider a range-bound prior that is rotated to be oriented along constant $\tau_{12}$ (see Fig.~\ref{fig:prior_box}, blue dashed rectangle). This rotated prior in $m_1-m_2$ space corresponds to a flat prior in $\tau_{12}$ space.
...
\eeq
CH: removing SRoll2 label in parenthesis and moving zmax=50 into the parenthesis.}
...
\ch{
\beq
\tau_{\rm PC}(15, 30) < 0.023 \; (95\%\; \mathrm{C.L.}).
\eeq
}
This should be contrasted with the tanh model 
 ...
 \wh{I might add a few words about the relationship to $\tau_{\rm hi}$ but first let's make sure that all upper limits are quoted as one sided 95\% since I'm not sure we're comparing apples-to-apples yet..}
...
%\beq
%\tau_{\rm PC}(15, 30)< 0.023 \; (95\%\; %\mathrm{C.L.})\;\;(\texttt{SRoll2}),
%\eeq
%[double check on full chain (from 10/15) this is from 09/30.]
%and 
\beq
\tau_{\rm PC}(15, 50) < 0.022 \; (95\%\; \mathrm{C.L.}).
\eeq
