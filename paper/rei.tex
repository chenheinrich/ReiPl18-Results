...
%
 \begin{figure}
          ...
          \caption{\textit{Top}: Rank-ordered CMB reionization principal components $S_a$ for $a = 1..5$ for $\zmax = 30$. We assume full ionized Universe at $z < 6$. \textit{Bottom}: The cumulative optical depth resulting from a unit amplitude PC.} 
          \label{fig:xe}
\end{figure}
...
\wh{Possibly add a few words on the PC version and refer to the
...
for each of its values, e.g. at $\tau_{12}=0.018$
than at $\tau_{12} = 0.125$.   Ref.~\cite{Millea:2018bko} suggest 
...
including these models to be large. \ch{CH: sorry, what do we mean by these models here?}  In any case, using PCs to 
