...
Our code is available on GitHub at \todo{give URL\url{...}}. The initial release corresponds to the fiducial results in this paper, i.e.\ derived from the Planck 2018 $\texttt{plik\_lite\_TTTTTE} + \texttt{lowl} + \texttt{SRoll2}$ likelihoods at $\zmax = 30$.  
...
Given the small amount of $\tau_{\rm hi}$ still allowed, the total optical depth constraints are correspondingly slightly larger than $\tau_{\rm tanh}$: \todo{give $\tau_{2\rm step}$.}
...
correspond to 
\beq
\tau_{\rm PC} = 0.0619^{+0.0056}_{-0.0068} \;\; (\texttt{SRoll2}),
...
the PC constraints gave \ch{$\tau=0.092\pm0.015$ which differed substantially from the tanh $\tau=0.079\pm0.017$~\cite{Heinrich:2016ojb}}.  As we shall discuss
...
%and 
\beq
\tau_{\rm PC}(15, 50) < 0.022 \; (95\%\; \mathrm{C.L.})\;\;(\texttt{SRoll2}).
\eeq
