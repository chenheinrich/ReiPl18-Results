...
\caption{Internal (with prior version)}
\label{fig:tau12}
\end{figure}
...
\begin{figure}[ht]
\includegraphics[width=0.5\textwidth]{/project2/whu5/chenhe/git/ReiPl18-Results/paper/plots/plot_tau12_new_apply_cut_False_vs_True_pl18_pc_zmax30_pliklite_srollv2_1015_two_panels.pdf}
\caption{$\tau_{12,new}$ \ch{(took out the cosmology part, only shrunk all posteriors very slightly and shifted them a bit to the left ); with prior version now added, it uses the mean from m3, m4, m5 that are computed from the chains with prior as well.}}
\label{fig:tau12}
\end{figure}
