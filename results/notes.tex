...
\caption{LATEST (to be debugged) -- 2-parameter model 2D plot from cosmomc KDE with f = 0.14 vs direct MCMC. They don't agree for low \taulo, in particular there is a strip around \tauhi = 0.02 that seems to have large likelihood (Note: perhaps I should pick a single point in that region to check that PC projection works fine and see if it is something with the PC chain when doing the KDE ... Also, there is a \taulo lower bound = 0.02 used just like for the direct MCMC, but giving getdist that limit doesn't help. We need this lower bound because otherwise the conversion \taulo, \tauhi to xemin and zre doesn't converge.)
}
\label{fig:}
\end{figure}
